#1
print("Press any key then press enter")
#2
print("Type your name and press enter")
#3
print("Type your age press enter")
#4
user_name = input("Type your name")
#5
user_age = int(input("Type your age"))
#6
user_movie = input("What is you favourite movie?")
#7
user_book = input("Type the name of a book")
#8
user_adjective = input("Type an adjective")
#9
user_noun = input("Type an noun")
#10
user_verb = input("Type an verb")
#11
print(f"Cool, I also like {user_movie} and {user_book}") 